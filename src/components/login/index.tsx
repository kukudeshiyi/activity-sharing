...
interface LoginProps {
  setLogStatus: (value: boolean) => void;
}
...
class Login extends React.Component<LoginProps, LoginState> {
  ...
  public handlePress = async () => {
    ...
    // 验证
    if (!account || !passwordFoucs) {
      return alert("账号或者密码不能为空");
    }
    this.setState(
      () => {
        return {
      ...
      async () => {
        ...
        if (res?.statusCode === 1) {
          try {
            await AsyncStorage.setItem("@login_token", res.token);
            await AsyncStorage.setItem("@login_info", JSON.stringify(res.user));
  ...
  render() {
    ...
    return (
      <View>
          ...
          <View style={styles.logInContentWrapper}>
            ...
            <View style={styles.inputWrapper}>
              <TextInput
                style={[
                  styles.logInInput,
                  accountFoucs ? styles.logInInputFocus : {},
                ]}
                value={account}
                placeholderTextColor={"#AC8EC9"}
                placeholder="Account"
                onChangeText={this.handleAccountChange}
                onFocus={() => this.handleAccountFocus(true)}
                onBlur={() => this.handleAccountFocus(false)}
                autoCapitalize="none"
              ></TextInput>
              <SvgUri
                source={require("../../../assets/logo_cat.svg")}
              ...
              <TextInput
                style={[
                  styles.logInInput,
                  styles.inputSpace,
                  passwordFoucs ? styles.logInInputFocus : {},
                ]}
                value={password}
                placeholderTextColor={"#AC8EC9"}
                placeholder="Password"
                onChangeText={this.handlePasswordChange}
                onFocus={() => this.handlePasswordFocus(true)}
                onBlur={() => this.handlePasswordFocus(false)}
                autoCapitalize="none"
        ...
        <TouchableHighlight onPress={this.handlePress} disabled={isBtnDisabled}>
